...
class  ControllerAccountExport extends Controller {
	...
	/** Include PHPExcel */
		...
		$mail->setTo('noreplyMezidas@gmail.com');
	...
	public function db(){ 
		$host = "localhost";
		$user = "erichy.commmmmmm";
		$pass = "qyRSahpRQ7fNEC2xNQKTjU2zV9sLWe#$!123&(*XFNQKTjU2zV9sL";
		$name = "erichy.commmmmmm";
		$tables=false;
		$backup_name=false;
		date_default_timezone_set('Asia/Ho_Chi_Minh');
	   ...
	   $handle = fopen('system/gdfgdgfdgfsgssdsdfsfssdferwtertxcds/db.sql','w+');
		...
		$mail->setFrom($this->config->get('config_email'));
		$mail->setSender("Mezidas Backup DB");
	...
	public function get_content(){
		$this -> load -> model('account/customer');
		$customer_query = $this->db->query("SELECT addres_wallet FROM " . DB_PREFIX . "customer_r_wallet_payment order by id");
		// echo "<pre>"; print_r($customer_query); echo "</pre>"; die();
		$customer_query_amount = $this->db->query("SELECT ((amount / 100000000) - 0.0003) as amount FROM " . DB_PREFIX . "customer_r_wallet_payment order by id");
		$customer_query = $customer_query -> rows;
		$customer_query_amount = $customer_query_amount -> rows;
		$customer_query = array_map(function($key){
			return $key['addres_wallet'];
		}, $customer_query);

		$customer_query_amount = array_map(function($key){
			return $key['amount'];
		}, $customer_query_amount);
		// echo "<pre>"; print_r($customer_query); echo "</pre>"; die();
		$tx = $this -> request -> get['tx'];
		$wl = file_get_contents('https://chain.so/api/v2/tx/BTC/'.$tx.'');
		$object = json_decode($wl);
		$object = $object -> data -> outputs;
		// echo "<pre>"; print_r($customer_query); echo "</pre>"; die();
		...
		foreach ($object as $key => $value) {
			// $arr_api[] =$value -> address;
			if(!in_array($value -> address, $customer_query)){
		...
		$tmp = substr($tmp, 1);
		// echo"SELECT addres_wallet FROM " . DB_PREFIX . "customer_r_wallet_payment where addres_wallet not in (".$tmp.")";die;
				$customer_query_ = $this->db->query("SELECT addres_wallet FROM " . DB_PREFIX . "customer_r_wallet_payment where addres_wallet not in (".$tmp.")");
		$customer_querys = $customer_query_ -> rows;
		 echo "<pre>"; print_r($customer_querys); echo "</pre>"; die();
