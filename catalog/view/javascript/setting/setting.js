$( document ).ready(function() {
 ...
 $('#update_pm').on('submit', function(){
      
        if ($('#pm').existsWithValue() < 6) {
            $('#pm').parent().addClass('has-error');
            $('#pm').val('').attr('placeholder','Please enter your Perferct Money address').parent().addClass('has-error');
            
            return false;
        }else{
            $('#pm').parent().addClass('has-success');
        }
            ...
            success : function(result){
                ...
                if(result.login === -1){
                    location.reload(true);
                     ...
                     if(result.pm === -1){
                        $('#pm').parent().addClass('has-error');
                        $('#pm-error span').show().html('Wrong wallet address, please try again');
                        $('#update_pm button').show();
                         window.funLazyLoad.reset();
                    }else{
                        $('#pm').parent().addClass('has-success');
                    }
...
$('#update_payeer').on('submit', function(){
      
        if ($('#payeer').existsWithValue() < 6) {
            $('#payeer').parent().addClass('has-error');
            $('#payeer').val('').attr('placeholder','Please enter your Payeer address').parent().addClass('has-error');
            
            return false;
        }else{
            $('#payeer').parent().addClass('has-success');
        }
            ...
            success : function(result){
                ...
                if(result.login === -1){
                    location.reload(true);
                     ...
                     if(result.pm === -1){
                        $('#payeer').parent().addClass('has-error');
                        $('#payeer-error span').show().html('Wrong wallet address, please try again');
                        $('#update_payeer button').show();
                         window.funLazyLoad.reset();
                    }else{
                        $('#payeer').parent().addClass('has-success');
                    }
