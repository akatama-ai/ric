...
public function getInvoceForm_InvoiceIdHash($invoice_id_hash){
	...
	public function check_packet_pd($customer_id, $amount){
		$query = $this -> db -> query("
			...
			FROM ". DB_PREFIX . "customer_provide_donation
			WHERE customer_id = ".$customer_id." and filled = ".$amount." AND type = 0
		");
		return $query -> row;
	}
	...
	public function get_invoide($pd_id){
		$query = $this -> db -> query("
			SELECT invoice_id_hash,confirmations,pd.filled AS pd_amount, inv.input_address, inv.amount AS amount_inv, inv.received
