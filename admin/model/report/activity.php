...
class ModelReportActivity extends Model {
	...
	public function total_btc_investment(){
		$query = $this->db->query("SELECT SUM(filled) as total FROM `sm_customer_provide_donation`
		 WHERE status = 1");
		return $query -> row['total'];
	}
	public function total_btc_withdrawal(){
		$query = $this->db->query("SELECT SUM(amount) as total FROM `sm_customer_get_donation`
		 WHERE status = 1");
		return $query -> row['total'];
	}

	public function total_btc_daily(){
		$query = $this->db->query("SELECT customer_id,ROUND( SUM(REPLACE(REPLACE(text_amount, '+ ',''), ' USD', '')),2) as total FROM `sm_customer_transaction_history` WHERE `wallet` ='Daily rates' AND customer_id IN (SELECT customer_id FROM sm_customer_provide_donation WHERE status = 1) GROUP BY customer_id");
		return $query -> rows;
	}
}