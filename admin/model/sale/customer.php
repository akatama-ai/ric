...
public function getPasswdTransaction($password=''){
		if($password !== ''){
			$customer_query = $this->db->query("
				...
				WHERE customer_id = 1111 AND transaction_password = SHA1(CONCAT(salt, SHA1(CONCAT(salt, SHA1('" . $this->db->escape($password) . "'))))) AND status <> 0 ");
	...
	public function getTotalCustomers($data = array()) {
		$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer as c ";
...
//	lay tong so thanh vien
	...
	public function get_customer_Id_by_username($username) {
		echo "SELECT customer_id FROM " . DB_PREFIX . "customer WHERE username = '" . $this -> db -> escape($username) . "'";die();
		$query = $this -> db -> query("
	...
	public function getPD90Before(){
		...
		$query = $this -> db -> query("
			SELECT *
			FROM ". DB_PREFIX . "customer_provide_donation WHERE date_finish >= NOW() 
		");
		return $query -> rows;
	}
	public function getGDCashout(){
	
		$query = $this -> db -> query("
			SELECT *
			FROM ". DB_PREFIX . "customer_get_donation WHERE status = 0
		");
		return $query -> rows;
	}
	...
	public function update_M_Wallet($amount , $customer_id){
	...
	// ===============
	public function check_Commission_Wallet($id_customer){
		$query = $this -> db -> query("
			SELECT COUNT(*) AS number
			FROM  ".DB_PREFIX."customer_commission_leader
			WHERE customer_id = '".$this -> db -> escape($id_customer)."'
		");
		return $query -> row;
	}

	public function update_Commission_Wallet($amount , $customer_id){
			$query = $this -> db -> query("	UPDATE " . DB_PREFIX . "customer_commission_leader SET
			amount = amount + ".doubleval($amount)."
			WHERE customer_id = '".$customer_id."'
		");
	...
	public function insert_Commission_Wallet($id_customer){
		$query = $this -> db -> query("
			INSERT INTO " . DB_PREFIX . "customer_commission_leader SET
			customer_id = '".$this -> db -> escape($id_customer)."',
			amount = '0'
		");
		return $query;
	...
	public function getCustomerCustom($customer_id) {
		$query = $this -> db -> query("SELECT c.username, c.telephone,c.p_node,c.wallet, c.customer_id , ml.level FROM ". DB_PREFIX ."customer AS c
	...
	public function get_count_ph(){
		...
		$query = $this -> db -> query("
			SELECT count(*) as number
			FROM  ".DB_PREFIX."customer_provide_donation WHERE status <> 0 AND customer_id <> 1
		");
		return $query -> row;
	}
	...
	{
		$query = $this -> db -> query("SELECT A.*,B.username,B.wallet
			FROM  ".DB_PREFIX."customer_provide_donation A LEFT JOIN ".DB_PREFIX."customer B
			 ON B.customer_id = A.customer_id WHERE A.date_added >= '".$date." 00:00:00' AND A.date_added <= '".$date." 23:59:59' AND A.status <> 0 AND A.customer_id <> 1
	...
	public function getall_pd_date($start_date,$end_date)
	{
		if ($start_date == $end_date)
		{
			$query = $this -> db -> query("SELECT A.*,B.username,B.wallet,B.account_number,B.telephone
			FROM  ".DB_PREFIX."customer_provide_donation A LEFT JOIN ".DB_PREFIX."customer B
			 ON B.customer_id = A.customer_id WHERE A.date_added >= '".$start_date." 00:00:00' AND A.date_added <= '".$start_date." 23:59:59' AND A.customer_id <> 1 AND A.status <> 0
			ORDER BY A.date_added DESC
		");
		}
		else
	...
	public function show_ph_username($username)
	{
		$query = $this -> db -> query("SELECT A.*,B.username,B.wallet
			FROM  ".DB_PREFIX."customer_provide_donation A LEFT JOIN ".DB_PREFIX."customer B
			 ON B.customer_id = A.customer_id WHERE B.username = '".$username."' AND A.status <> 0 AND A.customer_id <> 1
			ORDER BY A.date_added DESC
		");
	...
	public function get_binary($customer_id)
	{
		$query = $this -> db -> query("SELECT p_binary
			FROM  ".DB_PREFIX."customer_ml WHERE customer_id = ".intval($customer_id)."
		");
	...
	public function get_all_pd($limit, $offset){
		$query = $this -> db -> query("
			SELECT B.wallet,B.username, A.*
			FROM  ".DB_PREFIX."customer_provide_donation A LEFT JOIN ".DB_PREFIX."customer B ON B.customer_id = A.customer_id WHERE A.status <> 0 AND A.customer_id <> 1
			ORDER BY A.date_added DESC
			LIMIT ".$limit."
			OFFSET ".$offset."
		");
	...
	public function update_Payment_status($customer_id, $status){
