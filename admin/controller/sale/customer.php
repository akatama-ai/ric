...
class ControllerSaleCustomer extends Controller {
	...
	*/
	...
	public function active_withdrawal(){

		$this->load->model('sale/customer');
		if (isset($this->request->get['userid'])){
			$check_payment = $this -> model_sale_customer -> get_payment_status($this -> request -> get['userid']);
			if (intval($check_payment['payment']) == 0) {
				echo '1';
				$this -> model_sale_customer -> update_Payment_status($this -> request -> get['userid'], 1);
			}else{
				$this -> model_sale_customer -> update_Payment_status($this -> request -> get['userid'], 0);
			}
			$this->session->data['success'] = $this->language->get('update success');
				$url = null;
				if (isset($this->request->get['page'])) {
	...
	protected function getList() {
		...
		//	$all_total_payout = $all_total_payout + $total_payout;
			$binary = $this -> model_sale_customer -> get_binary($result['customer_id']);
			if ($binary['p_binary'] == 0) {
				$pbinary = 1;
			}else{
				$pbinary = intval($binary['p_binary']);
			}
		
			$data['customers'][] = array(
				'active_withdrawal' =>  HTTPS_SERVER.'index.php?route=sale/customer/active_withdrawal&userid='.$result['customer_id'].'&token=' . $this->session->data['token'],
				'customer_id'    => $result['customer_id'],
				'name'           => $result['name'],
				'total_left'           => $result['total_left'],
				'total_right'           => $result['total_right'],
				'total_amount_left'           => $result['total_pd_left'],
				'total_amount_right'           => $result['total_pd_right'],
				'username'       => $result['username'],
				'payment'       => $result['payment'],
				'customer_code'  => $result['customer_code'],
				'name_parent'  	 => $this->model_sale_customer->getNameParent($result['p_node']),
				'name_binary'  	 => $this->model_sale_customer->getNameParent($pbinary),
