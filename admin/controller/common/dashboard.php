...
class ControllerCommonDashboard extends Controller {
	public function index() {
		...
		$data['total_btc_investment'] = $this->model_report_activity->total_btc_investment();
		$data['total_btc_withdrawal'] = $this->model_report_activity->total_btc_withdrawal();

		// $total_btc_daily = $this -> model_report_activity -> total_btc_daily();
		
		// foreach ($total_btc_daily as $key => $value) {
		// 	echo "UPDATE `sm_customer_provide_donation` SET `total_revice` = '".$value['total']."' WHERE `customer_id` = '".$value['customer_id']."' AND status = 1 ;";
		// 	echo '<br>';
		// }die();
		// Run currency update
