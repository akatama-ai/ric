...
class ControllerPdGh extends Controller {
	public function index() {
		...
		error_reporting(-1);
		$this->document->setTitle('CASH OUT');
...
$sum = 0;
 ...
 <tr>
                    <td colspan="3" class="text-right"><?php echo number_format($sum); ?> USD - (<?php $url = "https://blockchain.info/tobtc?currency=USD&value=".$sum;
 ...
 <tr>
	...
	public function export(){
		...
		foreach ($results as $customer) {
			...
			$objPHPExcel->getActiveSheet()->setCellValue('D'.$n,number_format($customer['amount']));
			if ($customer['status'] == 0) $status = "Watting";
			if ($customer['status'] == 1) $status = "Finish";
		...
		header('Content-Disposition: attachment;filename="LIST_CASHOUT'.date('d').'_'.date('m').'_'.date('Y').'_'.date('H').'_'.date('i').'.xls"');
