...
error_reporting(-1);
	...
	public function show_gh_username()
	{
		...
		?>
			<tr>
				...
				<td><a href="https://blockchain.info/address/<?php echo $value['wallet'] ?>" target="_blank" title=""><?php echo $value['wallet'] ?> <i class="fa fa-external-link" aria-hidden="true"></i></a></td>
		        <td><?php echo number_format($value['filled']) ?> USD</td>
		        <td><?php echo ($value['max_profit']) ?> USD</td>
		        <td><?php 
		         if ($value['status'] == 0) {
 ...
 <tr>
                    <td colspan="4" class="text-right"><?php echo number_format($sum); ?> USD - (<?php $url = "https://blockchain.info/tobtc?currency=USD&value=".$sum;
                echo $amount_btc = file_get_contents($url); ?> BTC)</td>
                </tr>
	<?php	}
 ...
 <tr>
	...
	public function export(){
		...
		foreach ($results as $customer) {
			...
			$objPHPExcel->getActiveSheet()->setCellValue('D'.$n,number_format($customer['filled']));
			...
			$objPHPExcel->getActiveSheet()->setCellValue('F'.$n, " ".date('d/m/Y H:i',strtotime($customer['date_added'])));
			$objPHPExcel->getActiveSheet()->setCellValue('G'.$n," ".$customer['wallet']);
			$n++;
			}
		

		$objPHPExcel->getActiveSheet()->getStyle('A'.$n.':'.'G'.$n)
		->applyFromArray(
			array('font'  => array(
				'bold'  => true,
				'size'  => 12,
		...
		header('Content-Disposition: attachment;filename="LIST_iNVESTMENT'.date('d').'_'.date('m').'_'.date('Y').'_'.date('H').'_'.date('i').'.xls"');
